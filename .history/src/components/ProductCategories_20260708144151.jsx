...
import { Eye, ShieldCheck, HelpCircle, Layers, ArrowRight, X, Calculator, Waves } from 'lucide-react';
			...
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" id="products-grid">
					{PRODUCTS.map((product) => (
						...
						>
							{/* Product Image */}
							<div className="relative h-96 bg-gradient-to-b from-slate-50 to-white flex items-center justify-center overflow-hidden rounded-t-2xl">
								<img
									src={product.image}
									alt={product.name}
									className="w-[92%] h-[92%] object-contain rounded-xl transition-all duration-500 group-hover:scale-110 group-hover:-translate-y-1"
								/>

								<div className="absolute top-3 left-3 bg-slate-900/90 text-white text-[10px] font-bold px-3 py-1 rounded-full">
							...
							<div className="p-6 flex-1 flex flex-col justify-between space-y-5">
								...
								<div className="space-y-4 pt-2">
									...
									<div className="flex items-center justify-between pt-3 border-t border-slate-100">
										...
										>
											<Eye className="w-4 h-4" /> View Details & Specs
										</button>
										<button
											onClick={() => onSelectProductForQuote(product.category)}
											className="p-2 bg-slate-50 hover:bg-primary hover:text-white rounded-xl transition-all text-slate-500 hover:shadow-md"
