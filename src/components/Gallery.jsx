...
export default function Gallery() {
  const [filter, setFilter] = useState<'all' | 'plant' | 'products' | 'warehouse' | 'installation'>('all');
  ...
  const filterTabs = [
    ...
    { value: 'products', label: 'Water Tanks' },
    { value: 'warehouse', label: 'Warehouse & Loading' },
    { value: 'installation', label: 'Installation Sites' },
  ];
          ...
          <AnimatePresence mode="popLayout">
            {filteredItems.map((item, index) => (
              <motion.div
                ...
                className="group relative aspect-[3/2] overflow-hidden rounded-3xl border border-slate-200/60 bg-slate-100 shadow-xs hover:shadow-xl transition-all cursor-pointer"
              ...
              >
                ...
                {/* Overlay hover cover */}
                <div className="absolute inset-0 bg-slate-950/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-between p-5 text-white" />
                
                {/* Floating zoom indicator */}
                <div className="absolute top-4 right-4 bg-white/20 backdrop-blur-md p-2 rounded-xl text-white opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <ZoomIn className="w-4 h-4" />
                </div>

                {/* Info Text */}
                <div className="absolute bottom-0 left-0 right-0 p-5 translate-y-2 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-300 z-10 text-white">
                  <span className="text-[9px] font-bold text-secondary uppercase tracking-widest bg-white/10 px-2 py-0.5 rounded-md backdrop-blur-xs">
                    {item.category.toUpperCase()}
                  </span>
                  <h4 className="font-display font-bold text-sm mt-1 tracking-tight">
                    {item.title}
                  </h4>
                </div>
              </motion.div>
